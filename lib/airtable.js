...
// Site-wide category list — keep in sync with app/<slug>/page.jsx routes.
export const CATEGORIES = [
  { slug: 'olive-oils', label: 'Oils & Condiments' },
  { slug: 'grains',     label: 'Grains & Noodles' },
  { slug: 'legumes',    label: 'Legumes & Pulses' },
  { slug: 'snacks',     label: 'Snacks & Pantry' },
  { slug: 'lna',        label: 'Low & No Alcohol' },
...
/**
 * Fetch every approved product across all 6 categories, in parallel.
 * Each product is tagged with its category slug + display label so a
 * single flat list can be searched/filtered site-wide.
...
export async function getAllProducts() {
  ...
  return lists
    .flatMap((products, i) =>
      products.map(p => ({ ...p, category: CATEGORIES[i].slug, categoryLabel: CATEGORIES[i].label }))
    )
